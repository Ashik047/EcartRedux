    ...
    return (
        <div className='w-full h-[70px] bg-teal-900 flex justify-between items-center px-6 py-2'>
            ...
            <div className='flex gap-2 items-center'>
                {isSearch && <input onChange={e => (dispatch(searchProduct(e.target.value.toLowerCase())))} type="text" placeholder='Search Product Here' className='bg-white p-2 rounded-md me-10 w-[400px]' />}
                ...
                <Link to='/cart' className='font-semibold'><i className="fa-solid fa-cart-shopping text-blue-700" ></i>&nbsp;Cart <span className='bg-white p-1 rounded-sm'>{userCart?.length}</span></Link>
            </div>
        </div>
    )
}
